...
- 회원 가입 > profiles 에 insert 진행함.(오류발생)
- 회원 가입 > 이메일 인증 > 인증확인 > profiles 에 insert 필요.
...
function SignUpPage() {
  ...
  const handlSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      setMsg(
        '회원가입을 성공했습니다. 이메일 인증 링크를 확인해 주세요. 인증완료후 프로필이 자동으로 생성됩니다.',
      );
    }
  };
...
- 참조 : https://www.npmjs.com/package/react-paginate
- 우리는 직접 구현하기~!!!

## 1. 구현 시나리오

- 한 화면에 10 개의 목록을 표시함.(목표)
- 페이지 번호로 네비게이션 함.

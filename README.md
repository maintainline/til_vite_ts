...
- 기존 todos 테이블 삭제
- 추가 칼럼으로 auth.users 의 id(uuid) 를 FK 로 칼럼 설정
...
CREATE TABLE todos (
    ...
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    -- user_id 는 uuid 로서 auth.users의 id를 참조하고 사용자 제거시 같이 삭제
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,

    -- titile 칼럼은 비어있으면 안되고, 글자이다.
    title VARCHAR NOT NULL,
...
-- RLS 활성화
-- todos 테이블은 활성화 RLS로 적용 하라.
ALTER TABLE todos ENABLE ROW LEVEL SECURITY;
```

## 2. RLS 적용 후 Policy 정책을 진행해 주어야 함.

- todos 의 보안 정책을 작성해서 , CRUD 가능하도록 적용

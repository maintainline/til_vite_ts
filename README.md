...
- `데이터베이스 비밀 번호`는 꼭 보관하자~! (.env)
- 테이블 생성 (Table Editor / SQL Editor)
- 기본형은 `Table Editor` 로 생성하고 추가적 설정은 `SQL Editor` 활용
- 익숙하다면 `SQL Editor` 로 관리 권장 (Chat GPT 활용 권장)
...
```
VITE_SUPABASE_DB_PW=데이터 베이스 비밀번호
VITE_SUPABASE_URL=url 값
VITE_SUPABASE_ANON_KEY=키값
```

- Supabase URL 과 Anon Key 파악하기
  - project 선택후 `Project overview` 에서 확인
  - `project API` 항목에서 확인 가능

- <img width="1442" height="779" alt="Image" src="https://github.com/user-attachments/assets/437c34f2-741e-47bb-adb4-7e6157f6ce72" />
...
- `id` 는 URL 의 앞쪽 단어가 ID 가 됨.
  ...
  - https://`여기 이부분~`.supabase.co
  - project setting 에서 확인가능
  - <img width="1405" height="739" alt="Image" src="https://github.com/user-attachments/assets/5edcbd42-d224-4785-8094-52dc60d27e3d" />
    -package.jsoon 추가

```json
"generate-types": "npx supabase gen types typescript --project-id 프로젝트 아이디 --schema 경로 > 파일명.ts"
```
...
```json
 "scripts": {
     ...
    "generate-types": "npx supabase gen types typescript --project-id ewdkdkmxuasaqeuhrjnl --schema public > types_db.ts"
  },
```
...
- 참조 : https://supabase.com/docs/reference/javascript/start
- `/src/apis` 폴더 생성 또는 `/src/services` 폴더 생성
- `/src/services 폴더`로 진행.
- `/src/services/todoServices.ts 파일` 생성.
- 반드시 `async ... await 활용(비동기)`
- 반드시 함수 리턴타입은 `Promise<리턴 데이터 타입>`
  - axios, fetch 등등...
...
- int2(Smail Integer) : 작은 숫자 (-32768~32767)
...
- numeric (Decimal) : 엄청 정밀한 소숫점 값 (예: 1234.141519)

## 2. String 종류

- text : 가장 많이 사용하는 타입, 길이 제한이 없음 (예 : html 등)
- varchar(숫자) : 길이를 지정함. 남는 공간도 포함시켜버림 (예: 이름, 이메일, 주소 등)

## 3. Boolean 종류

- bool : true / false 값만 가질 수 있음 (예: 주문완료, 주문취소 등)

## 4. JSON 종류(구조화 된 즉 `{키명 : 값...}`)

- json : 글자입니다.(예 : `{이름:아이유, 나이:30,...}`)
- jsonb : 더 빠르게 검색이 가능한 형태의 바이너리 코드로 변환한 자료형.
- 참고 사항으로 Firebase, MongoDB 에서 이 타입을 사용함.

## 5. 독특한 데이터 타입

- uuid : 고유한 id 로 저장할 때 (`pk` 로 추천)
- bytea : 파일, 이미지 등을 저장할 수 있다.(사용을 하진 않음. x / 그래서 권장하지 않음)

## 6. Date & Time (날짜와 시간 데이터)

- date : 연, 월, 일(예:2025-09-19)
- time : 시간만 저장 (예 : 19:09 2025-09-19 19
- timetz : 시간대(예:19:09: 2025-09-19 19)
...
- 처음에는 RLS 에대해서 적용을 하지 않고 개발을 시작한다.
- 하지만, 결국 RLS 가 필요하므로 미리 셋팅에 대해서 고민을 하여야 한다.

## 1. RLS 정의 단계

### 1 단례 : `테이블을 먼저 정의 및 생성` 한다.
...
- `이름 문자열` 은 중복해서 여러 번 생성 불가

```sql
create policy "이름 문자열"
on 테이블명
...
```sql
alter table profile add column 칼럼명 데이터종류
...
```sql
create policy "Guest can only read public memos"
on memos
for sekect

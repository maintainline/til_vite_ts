...
- 기본 제공되는 탈퇴기능
  ...
  - 관리자 전용 (서버에서만 실행됨)
  - react 는 클라이언트 즉, 웹 브라우저 전용이라서 실행 불가
  - 보안상 위험 : 실수로 지울 가능성
  - 복구 불가
- 탈퇴 기능
  - 사용자 비활성
...
-- 탈퇴 신청한 사용자 목록 테이블
CREATE TABLE account_deletion_requests (
  -- PK 이고, DELFAULT gen_random_uuid() 중복 안되는 ID 생성
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const deleteAccount: AuthContextType['deleteAccount'] = async () => {
    try {
      ...
      return {
        success: true,
        message: '계정삭제가 요청되었습니다. 관리자 승인 후 완전히 삭제 됩니다.',
      };
    } catch (err) {

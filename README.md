...
export interface DirectChat {
  ...
  last_message_at: string; // 마지막 메세지 시간
  user1?: ChatUser; // 참여자 1번의 정보
  user2?: ChatUser; //  참여자 2번의 정보
  last_message?: DitrectMessage; //  마지막 메세지 정보
}

// 1:1 메세지 타입
export interface DitrectMessage {
...
// 메세지의 상세 추가 확장 정보
export interface MessageDetail extends DitrectMessage {
  sender: ChatUser;
}
...
export interface ChatListItem {
  ...
  last_message?: {
    content: string; // 마지막 메세지 내용
    created_at: string; // 작성시간
    sender_nickname: string; // 보낸사람 닉네임
  };
  unread_count: number; // 읽지않은 메세지 수
}

// 채팅방 생성용
export interface CreateChatData {
  participant_id: string; // 상대방 사용자 아이디 ID
}
...
export interface ChatState {
  ...
  messages: DitrectMessage[]; // 메세지 목록
  loading: boolean; // 로딩상태
  error?: string; // 에러 메세지
}
...
- `src/service/chat/directChatService.ts`

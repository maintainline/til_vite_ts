  ...
  return (
    <div className="chat-page">
      ...
      <div className="chat-container">
        {/* 왼쪽 사이드바 - 채팅 목록 */}
        <div className="chat-sidebar">DirectChatList</div>
        {/* 오른쪽 메인영역 - 채팅방 또는 환영 인사 표시 */}
        <div className="chat-main">메인</div>
      </div>
    </div>
  );
}
...
- `/src/components/chat/chat.css` 생성

```css
/* 채팅 페이지 전체 컨테이너: 헤더 높이를 제외한 전체화면 높이 사용*/
.chat-page {
...
/* search-result 검색 결과 목록 : 스크롤 가능한 최대 높이 설정*/
.search-result {
  margin: 8px;
  max-height: 200px;
  overflow-y: auto;
...
/* avater-placeholder - 이미지가 없을때 이니셜 표시 */
.avater-placeholder {
  width: 32px;
  height: 32px;
  border-radius: 50%;
...
/* 채팅 상대방 아바타 컨테이너 - 읽지않은 메시비 배치 위치 기준 */
.chat-avatar {
  position: relative;
  margin-right: 16px;
}
...
/*  메세지 그룹 - 같은 날짜의 메세지들은 묶음 */
.message-group {
  margin-bottom: 24px;
}
/* 날짜 구분선 - 메세지 그룹 사이에 날자 표시 */
.date-divider {
  text-align: center;
  margin: 16px 0;
  position: relative;
...
- 자동 높이 조절되는 텍스트 영역
- enter 키로 메세지 전송, shift +enter 로 줄바꿈
- 전송중 로딩 상태 표시
- 빈메세지 전송 방지
- 전송 후 입력 필드 자동 초기화
*/
  ...
  return (
    <div className="message-input">
      <form className="message-form">
        {/* 입력 컨테이너- 텍스트 영역과 전송버트 */}
        <div className="input-container">
          {/* 메세지 입력 텍스으 */}
          <textarea
            className="message-textarea"
            rows={1}
            placeholder="메세지를 입력하세요...(Enter로 Shift+enter롤 전송간으"
          />
          {/* 메시지 전송 버튼 */}

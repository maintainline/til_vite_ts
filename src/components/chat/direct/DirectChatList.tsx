...
const DirectChatList = ({ onChatSelect, onCreateChat, selectedChatId }: DirectChatListProps) => {
  //  DB 에서 읽어온 데이터를 관리함:여러곳에서 활용하는 데이터 이므로 context 를 활용예정
   ...
   *처리 과정
   1. 선택된 사요자와 새채팅방 생성
   2. 생성된 채팅방을 즋 선택된 것으로 인정
   3. 사용자 검색ui 숨김
   4 . 사용자 검색어 초기화
   */
  const hanldeUserSelect = (user: ChatUser) => {
    // 사용자 선택됨
    // 사용자의 id 를 이용해서 채팅방을 생성해야 합ㄴ디ㅏ.

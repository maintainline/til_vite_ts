...
// 컴포넌트가 외부에서 전달 받을 데이터 형태
interface RichTextEditorProps {
  value: string; // 에디터의 초기로 보여줄 내용
  onChange: (value: string) => void; // 내용이 변경될때 실행할 함수.
  placeholder?: string; // 안내 텍스트(선택사항.)
  disabled?: boolean; // 에디터를 비활성화 할지 여부(선택사항.)
  // 추가됨.
...
const RichTextEditor = ({
  ...
  //  ref 변수들을 저장해둠.
  //reactQuill 을 보관해 둡니다.
  const quillRef = useRef<ReactQuill | null>(null);

  // 미리보기 이미지들을 보관할 임시 목록('blon :~')
  const tempImagesRef = useRef<TempImageFile[]>([]);

  // 가장 최근의 내용을 관리하기 위한 변수
  const valueRef = useRef<string>(value);

  // 임시이미지 url 을 생성하는 기능(기능을 한번만 만들고 재활용 useCallback)
    ...
    input.onchange = async () => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (file.size > 5 * 1024 * 1024) {
          alert(`${file.name}은 이미지 파일크기는 5MB 이하여야 합니다..`);
  ...
  const syncTempImages = useCallback(() => {
    //현재 에디터 내용에 사용중인 tempUrl 을 추출함.
    // 데이터 타입에서 Arr 처럼 set 도 있습니다..
    const usedTempUrls = new Set<string>();
    //내용에서 blob으로 된 글자를 찾아줄 겁니다.
    // 글자들을 비교할때 정규 표현식(Regular Expression)을 사용함.
    ...
    //     // 내용에 임시 미리보기 url 글자가 없다면 삭제해야한다.
    //     if (!isUsed) {
    //       // 사용하지 않는 bolb URL 정리하기
    ...
    tempImagesRef.current.forEach(item => {
      if (!usedTempUrls.has(item.tempUrl)) {
        ...
        URL.revokeObjectURL(item.tempUrl);
        console.log(`이미지 삭제 됨 : ${item.id} ${item.tempUrl}`);
      }
    });
    // 에디터 순서대로 재 정령된 배열로 업데이트
    tempImagesRef.current = orderdImages;
  }, []);

  // 에디터의 내용이 변경되면 임시 이미지 동기화.
  useEffect(() => {
    // 메모리 누수방지 및 필요없는 파일 업로드 방지용..
    syncTempImages();
